...
@pytest.fixture
async def pipeline(mongo_db, monkeypatch):
    """The pipeline module against a throwaway database, Mastra stubbed."""
    from app.services import processing_service, us_mastra_client
    ...
    yield processing_service, mongo_db, us_mastra_client, monkeypatch


async def _seed(mongo_db, country="USA", doc_type=None):
    ...
    await mongo_db.documents.insert_one({
        **_DOCUMENT_DEFAULTS,
        ...
        "pages": [{
            "id": "p1", "document_id": "doc-1", "page_number": 1,
            "original_path": "/tmp/orig.png", "preprocessed_path": "/tmp/prep.png",
            "width": None, "height": None, "created_at": "2026-01-01T00:00:00",
        }],
    })


async def _document(mongo_db, document_id="doc-1"):
...
class TestUsPipeline:
    async def test_a_shipping_authorization_runs_end_to_end(self, pipeline):
        processing_service, mongo_db, us_client, mp = pipeline
    ...
    async def test_a_type_already_set_is_respected(self, pipeline):
        ...
        processing_service, mongo_db, us_client, mp = pipeline
        await _seed(mongo_db, doc_type="SA")
    ...
    async def test_an_invoice_goes_to_the_invoice_extractor_and_rules(self, pipeline):
        ...
        assert result["status"] == "VALID"
        assert (await _document(mongo_db))["doc_type"] == "INV"
    ...
    async def test_an_india_document_still_takes_the_india_path(self, pipeline):
        ...
        processing_service, mongo_db, us_client, mp = pipeline
        await _seed(mongo_db, country="INDIA")
