...
async def _merge_into_extraction_json(document_id: str, patch: dict) -> None:
    """Merge keys into the same brs_json blob that already holds bank_statement, coa, ledger, etc. — no separate table."""
    now = datetime.utcnow().isoformat()
    async with get_db() as db:
        cursor = await db.execute(
            "SELECT id, brs_json FROM brs_extraction_results WHERE document_id = ? ORDER BY created_at DESC LIMIT 1",
            (document_id,),
        )
        row = await cursor.fetchone()
        if row:
            brs_json = json.loads(row["brs_json"]) if row["brs_json"] else {}
            brs_json.update(patch)
            await db.execute(
                "UPDATE brs_extraction_results SET brs_json = ? WHERE id = ?",
                (json.dumps(brs_json), row["id"]),
            )
        else:
            result_id = str(uuid.uuid4())
            await db.execute(
                "INSERT INTO brs_extraction_results (id, document_id, brs_json, created_at) VALUES (?, ?, ?, ?)",
                (result_id, document_id, json.dumps(patch), now),
            )
        await db.commit()
