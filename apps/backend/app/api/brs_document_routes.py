...
async def process_brs_document(document_id: str):
    ...
    # Step 2b: Plain bank-statement extraction (transaction ledger), all pages, batched
    await docs.log_step(document_id, "EXTRACTION", "STARTED", "Calling bank statement vision agent")
    bank_statement_result = await brs_mastra_client.call_bank_statement_agent({
        "document_id": document_id,
        "page_image_paths": preprocessed_paths,
        "ocr_text": ocr_text,
    })
    brs_json["bank_statement"] = bank_statement_result.get("bank_statement", {})
    brs_json["metadata"]["bank_statement_confidence"] = bank_statement_result.get("confidence", {})

    failed_batches = bank_statement_result.get("failed_batches") or []
    total_batches = bank_statement_result.get("total_batches", 0)
    if failed_batches:
        await docs.log_step(
            document_id, "EXTRACTION", "WARNING",
            f"Bank statement extraction failed for {len(failed_batches)}/{total_batches} page batch(es) "
            f"after retries: batch(es) {failed_batches}. Transactions from those pages are missing.",
        )
